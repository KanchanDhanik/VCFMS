...
LANGUAGES = {
    "English": {
        ...
        "phrases": {
            ...
            "what_to_open": "What would you like to open?",
            "what_to_delete": "What would you like to delete?",
            "what_to_rename": "What would you like to rename?",
            "what_to_copy": "What would you like to copy?",
            "what_to_move": "What would you like to move?",
            "what_to_preview": "What file would you like to preview?",
            "new_name": "What would you like to rename it to?",
    ...
    "Hindi": {
        ...
        "phrases": {
            ...
            "what_to_open": "आप क्या खोलना चाहेंगे?",
            "what_to_delete": "आप क्या हटाना चाहेंगे?",
            "what_to_rename": "आप किसका नाम बदलना चाहेंगे?",
            "what_to_copy": "आप क्या कॉपी करना चाहेंगे?",
            "what_to_move": "आप क्या मूव करना चाहेंगे?",
            "what_to_preview": "आप किस फाइल का प्रीव्यू देखना चाहेंगे?",
            "new_name": "आप इसे क्या नाम देना चाहेंगे?",
            "item_not_found": "आइटम नहीं मिला। कृपया फिर से कोशिश करें।"
        }
    }
}
...
pending_command = None  # To store the command that needs a target
pending_target = None   # To store the target of a command
...
# Function to find an item by name in the current directory
def find_item_by_name(name):
    current = current_dir.get()
...
def rename_item(item_path=None):
...
def delete_item(item_path=None):
    if item_path is None:
        selected_item = tree.focus()
        if not selected_item:
            messagebox.showwarning("Warning", "No file or folder selected")
...
def preview_file(item_path=None):
    if item_path is None:
        selected_item = tree.focus()
        if not selected_item:
...
# Function to process voice commands with two-step interaction
def process_command(command):
    ...
    # If we have a pending command, process it as a target
    if pending_command:
        target = command
        target_path = find_item_by_name(target)
        
        if target_path:
            # Execute the pending command with the target
            if pending_command == "open_file":
                if os.path.isdir(target_path):
                    open_item('folder', target_path)
                else:
                    open_item('file', target_path)
            elif pending_command == "open_folder":
                if os.path.isdir(target_path):
                    open_item('folder', target_path)
                else:
                    speak("select_folder")
            elif pending_command == "delete":
                delete_item(target_path)
            elif pending_command == "rename":
                rename_item(target_path)
            elif pending_command == "copy":
                copy_item(target_path)
            elif pending_command == "move":
                move_item(target_path)
            elif pending_command == "preview":
                preview_file(target_path)
        else:
            speak("item_not_found")
    ...
    if cmd_set["open_file"] in command:
        pending_command = "open_file"
        speak("what_to_open")
        start_listening()
    elif cmd_set["open_folder"] in command:
        pending_command = "open_folder"
        speak("what_to_open")
        start_listening()
    elif cmd_set["rename"] in command:
        pending_command = "rename"
        speak("what_to_rename")
        start_listening()
    elif cmd_set["copy"] in command:
        pending_command = "copy"
        speak("what_to_copy")
        start_listening()
        ...
        speak("what_to_delete")
        start_listening()
    elif cmd_set["preview"] in command:
        pending_command = "preview"
        speak("what_to_preview")
        start_listening()
